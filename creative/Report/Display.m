...
for h=1:Number_of_Frames 
    ...
    if (h == 5)
        Agent_Death_Chance = 0.1;
    end
    
    if (h == 7)
        Cop_Death_Chance = 0.1;
        L = L - 0.3;
    end
    ...
    for i=1:1:side
        ...
        for j=1:1:side
            ...
            if strcmp(A.Get_Type(), 'Agent')
                ...
                Colors_Attitude((i-1)*side + j) = max(-1, A.Grievance(L) - A.Net_Risk(A.Chance_of_Arrest(k, Cops, Agents), Jailterm_Max, Better_Deterrent_of_Jailtime));
